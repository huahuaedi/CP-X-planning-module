...
def _resolve_traffic_manager_port(traffic_config):
    env_port = os.environ.get("OPENCDA_TRAFFIC_MANAGER_PORT", "").strip()
    if env_port:
        return int(env_port)
    return int(traffic_config.get("port", traffic_config.get("tm_port", 8000)))
...
class ScenarioManager:
    ...
    def create_traffic_carla(self):
        ...
        traffic_manager_port = _resolve_traffic_manager_port(traffic_config)
        bind_error = None
        tm = None
        for candidate_port in range(int(traffic_manager_port), int(traffic_manager_port) + 21):
            try:
                tm = self.client.get_trafficmanager(int(candidate_port))
                if int(candidate_port) != int(traffic_manager_port):
                    print(
                        'Traffic Manager port %d was busy; using %d instead.'
                        % (int(traffic_manager_port), int(candidate_port))
                    )
        ...
        if tm is None:
            if bind_error is not None:
                raise bind_error
            raise RuntimeError('Could not create CARLA Traffic Manager.')
