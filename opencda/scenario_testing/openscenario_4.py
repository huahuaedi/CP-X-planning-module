...
def run_scenario(opt, scenario_params, experiment_params=None):
    ...
    if experiment_params:
        for profile in experiment_profile.profiles():
            if profile in experiment_params:
                scenario_params = OmegaConf.merge(
                    scenario_params, experiment_params[profile])
    ...
    try:
        ...
        if experiment_profile == Profile.PREDICTION_OPENCOOD_CAV:
            ...
            if other_cav_list or len(scenario_params['scenario']['single_cav_list']) > 1:
                single_cav_list = scenario_manager.create_vehicle_manager_openscenario(
                    application=['single'], vehicles=[ego_vehicle] + other_cav_list
                )
            else:
