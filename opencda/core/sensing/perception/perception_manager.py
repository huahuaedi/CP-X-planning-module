...
# Open3D library imports
from opencda.core.sensing.perception.o3d_lidar_libs import \
    o3d_visualizer_init, o3d_pointcloud_encode, o3d_visualizer_show, \
    o3d_camera_lidar_fusion

# Commented out PyVista
# from opencda.core.sensing.perception.pyvista_lidar_libs import \
#     pv_visualizer_show, pv_visualizer_init, pv_camera_lidar_fusion, get_pv_pointcloud_data
...
class PerceptionManager:
    ...
    def __init__(self, vehicle, config_yaml, cav_world,
        ...
        # perception scenarios can run with the current manager.
        camera_config = config_yaml['camera'] if 'camera' in config_yaml else {}
        lidar_config = config_yaml['lidar'] if 'lidar' in config_yaml else {}
        ...
        self.camera_visualize = camera_config['visualize'] \
            if 'visualize' in camera_config else config_yaml.get('camera_visualize', 0)
        self.camera_num = camera_config['num'] \
            if 'num' in camera_config else config_yaml.get('camera_num', 0)
        if 'positions' not in camera_config and self.camera_num:
            camera_config['positions'] = default_camera_positions[:self.camera_num]

        self.lidar_visualize = lidar_config['visualize'] \
            if 'visualize' in lidar_config else config_yaml.get('lidar_visualize', False)
        self.lidar_save_data = lidar_config.get('save_data', False)
        if str(os.environ.get("OPENCDA_DEBUG_VIEW", "")).strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
        ...
        if self.activate and not ml_manager:
            print('[OpenCDA Perception] apply_ml=False but perception.activate=True; '
                  'falling back to CARLA ground-truth perception.')
            self.activate = False
        ...
        if self.activate or self.camera_visualize:
            self.rgb_camera = []
            mount_position = camera_config['positions']
        ...
        # we only spawn the LiDAR if visualization or saving is needed
        if self.activate or self.lidar_visualize or self.lidar_save_data:
        ...
        # <-- FIX: Renamed pv_plotter to o3d_vis and use O3D init -->
    ...
    def activate_mode(self, objects):
        ...
        # <-- FIX: Corrected visualization logic for Open3D -->
        if self.lidar_visualize and self.lidar is not None:
            while self.lidar.data is None:
                continue
            
            # Encode data into the persistent o3d_pointcloud object
        ...
        # <-- MODIFICATION: Hard-coded save logic -->
        if self.lidar_save_data and self.lidar is not None:
            while self.lidar.data is None:
                continue
            # Encode the raw data into the o3d object
            o3d_pointcloud_encode(self.lidar.data, self.lidar.o3d_pointcloud)
            # Save the o3d object
            #save_point_cloud_data(self.count, self.lidar.o3d_pointcloud)
        # <-- END MODIFICATION -->
    ...
    def deactivate_mode(self, objects):
        ...
        if self.lidar_visualize and self.lidar is not None:
            ...
            # render the raw lidar with Open3D
            o3d_visualizer_show(
                self.o3d_vis,
                self.count,
                self.lidar.o3d_pointcloud,
    ...
    def _get_active_light(self, tl_list, vehicle_location, vehicle_waypoint):
        for tl in tl_list:
            ...
            # <-- FIX: Corrected typo self.map to self._map -->
    ...
    def destroy(self):
        ...
        # <-- FIX: Use correct Open3D variable and destroy method -->
        if self.lidar_visualize and self.o3d_vis:
...
# <-- MODIFICATION: Added this helper function for saving -->
def save_point_cloud_data(count, point_cloud, save_dir="/home/umd-user/Desktop/lidar_data"):
    """
    Saves the Open3D point cloud to a .pcd file.

    Parameters
    ----------
    count : int
        Current frame/step number, used for the filename.

    point_cloud : o3d.PointCloud
        The Open3D point cloud object to save.
    ...
    # Define a unique filename based on the count (e.g., lidar_frame_000001.pcd)
    filename = os.path.join(save_dir, f"lidar_frame_{count:06d}.pcd")
    
    # Save the point cloud
    o3d.io.write_point_cloud(filename, point_cloud)
    
    #Print confirmation message
    print(f"Lidar data saved: {filename}")
...
# import sys
# import time
# import os  # <-- MODIFICATION: Added import
# import carla
# import cv2
# import numpy as np
# import open3d as o3d  # <-- MODIFICATION: Re-enabled import
...
#     ObstacleVehicle
# from opencda.core.sensing.perception.static_obstacle import TrafficLight
# # Removed Open3D library imports
# from opencda.core.sensing.perception.o3d_lidar_libs import \
#     o3d_visualizer_init, o3d_pointcloud_encode, o3d_visualizer_show, \
#     o3d_camera_lidar_fusion
...
#         # <-- MODIFICATION: Added o3d_pointcloud object back for saving -->
#         self.o3d_pointcloud = o3d.geometry.PointCloud()
...
#     pv_plotter : pv.Plotter object
#         PyVista point cloud visualizer.
#     """
...
#         #self.camera_visualize = False
#         self.camera_num = config_yaml['camera']['num']
#         #self.lidar_visualize = config_yaml['lidar']['visualize']
#         self.lidar_visualize = False
#         # <-- MODIFICATION: Added save_data flag -->
#         self.lidar_save_data = config_yaml['lidar'].get('save_data', True)
...
#         # <-- MODIFICATION: Added save_data flag to condition -->
#         if self.activate or self.lidar_visualize or self.lidar_save_data:
#             self.lidar = LidarSensor(vehicle,
#                                      self.carla_world,
#                                      config_yaml['lidar'],
...
#         # <-- MODIFICATION: Only init plotter if visualize is true -->
#         if self.lidar_visualize:
#             # PyVista plotter initialization
#             self.pv_plotter = o3d_visualizer_init(self.id)
#         else:
...
#         -------
#          objects: dict
#             Updated object dictionary.# def save_point_cloud_data(count, point_cloud, save_dir="/home/umd-user/Desktop/lidar_data"):
#     """
#     Saves the Open3D point cloud to a .pcd file.
...
#             # camera lidar fusion - Switched to PyVista version
#             objects = o3d_camera_lidar_fusion(
#                 objects,
#                 yolo_detection.xyxy[i],
#                 self.lidar.data,
...
#             # Get points and colors for PyVista
#             points, colors = o3d_pointcloud_encode(self.lidar.data)
...
#         # <-- MODIFICATION: Added save data logic -->
#         if True:
#             while self.lidar.data is None:
#                 continue
#             # Encode the raw data into the o3d object
...
#             # render the raw lidar with PyVista
#             o3d_visualizer_show(
#                 self.pv_plotter,
#                 self.count,
#                 points,
...
#         if self.lidar_visualize and self.pv_plotter:
#             # Close the PyVista plotter
...
# # # --- MODIFIED FUNCTION ---
# # # WARNING: This function will be VERY SLOW.
# # def save_point_cloud_data(count, point_cloud, save_dir="/home/umd-user/Desktop/lidar_data"):
# #     """
# #     Saves all point clouds to a single, merged file.
# #     This is INEFFICIENT and will be slow, as it reads and writes
# #     the entire file every time it's called.
...
# #     point_cloud : o3d.PointCloud
# #         The new Open3D point cloud object to add.
# #     save_dir : str
# #         The directory where the single merged file will be saved.
# #     """
    
# #     # Define the single file path
# #     save_file_path = os.path.join(save_dir, "merged_lidar.pcd")
...
# #         # If it exists, read it. This is the slow part.
# #         try:
# #             merged_pcd = o3d.io.read_point_cloud(save_file_path)
# #         except Exception as e:
# #             print(f"Warning: Could not read existing file {save_file_path}. Will overwrite. Error: {e}")
# #             merged_pcd = o3d.geometry.PointCloud()
    
# #     # 2. Add the new (current frame) point cloud data to the merged cloud
# #     #    The '+' operator combines them
# #     merged_pcd += point_cloud
    
# #     # 3. Save the new, combined point cloud back to the same file. This is also slow.
# #     o3d.io.write_point_cloud(save_file_path, merged_pcd)
    
# #     # 4. Print confirmation message
# #     print(f"Lidar data frame {count} appended to: {save_file_path}")
# # # --- END MODIFICATION ---
# # # <-- END MODIFICATION -->
...
# import cv2
# import numpy as np
# # import open3d as o3d  # Removed Open3D import
...
#         self.timestamp = None
#         self.frame = 0
#         # open3d point cloud object - Removed
#         # self.o3d_pointcloud = o3d.geometry.PointCloud()
...
#                                      self.global_position)
#             # PyVista plotter initialization
#             self.pv_plotter = pv_visualizer_init(self.id)
#         else:
#             self.lidar = None
#             self.pv_plotter = None
...
#             # camera lidar fusion - Switched to PyVista version
#             objects = pv_camera_lidar_fusion(
#                 objects,
#                 yolo_detection.xyxy[i],
#                 self.lidar.data,
#                 projected_lidar,
...
#             # Get points and colors for PyVista
#             points, colors = get_pv_pointcloud_data(self.lidar.data)
