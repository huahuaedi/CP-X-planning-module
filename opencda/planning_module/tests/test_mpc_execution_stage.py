...
def test_hard_gate_resets_buffer_and_carries_no_control():
    # Whether a hard gate is an emergency is ControlFinalizationStage's call
    # (see test_control_finalization_stage); this stage only vetoes the plan.
    ...
    assert result.fallback_reason == "candidate_hard_gate:reference_invalid"
    assert buffer.reset_reason == "control_buffer_reference_hard_veto"
    assert mpc.plan_kwargs == {}, "a hard-gated reference must never reach the solver"
