...
class RunnerObstacleFilterTests(unittest.TestCase):
    ...
    def test_merge_tracker_predictions_attaches_future_trajectory_to_snapshot(self):
        merged = _merge_tracker_predictions(
            object_snapshots=[
                {
                    "vehicle_id": "car1",
                    "x": 1.0,
            ...
            predictions={
                "car1": [
                    {"x": 2.0, "y": 2.0, "v": 3.0, "psi": 0.0},
                    {"x": 3.0, "y": 2.0, "v": 3.0, "psi": 0.0},
                ]
            },
        )

        self.assertEqual(
            merged[0]["predicted_trajectory"],
            [[2.0, 2.0, 3.0, 0.0], [3.0, 2.0, 3.0, 0.0]],
        )
    ...
    def test_lane_safety_assignment_keeps_connector_obstacle_in_ego_lane_inside_intersection(self):
        assigned_lane_id = _lane_safety_assignment_for_obstacle(
            ...
            ego_snapshot={"x": 0.0, "y": 0.0, "psi": 0.0},
            obstacle_snapshot={"x": 0.0, "y": 0.1},
        )

        self.assertEqual(assigned_lane_id, 2)

    def test_lane_safety_assignment_falls_back_to_relative_lane_when_context_lane_is_invalid(self):
        assigned_lane_id = _lane_safety_assignment_for_obstacle(
            ego_lane_context={
                "road_id": "12:0",
                "road_numeric_id": 12,
            ...
            available_lane_ids=[1, 2],
            ego_snapshot={"x": 0.0, "y": 0.0, "psi": 0.0},
            obstacle_snapshot={"x": 5.0, "y": 3.6},
        )

        self.assertEqual(assigned_lane_id, 2)

    def test_lane_safety_assignment_uses_local_geometry_when_road_ids_disagree_but_heading_and_lane_band_match(self):
        assigned_lane_id = _lane_safety_assignment_for_obstacle(
            ego_lane_context={
                "road_id": "12:0",
                "road_numeric_id": 12,
            ...
            available_lane_ids=[1, 2],
            ego_snapshot={"x": 0.0, "y": 0.0, "psi": 0.0},
            obstacle_snapshot={"x": 12.0, "y": 3.4, "psi": 0.02},
        )
