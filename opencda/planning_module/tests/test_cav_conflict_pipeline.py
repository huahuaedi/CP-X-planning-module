...
def _peer(actor_id, xy, committed_at_s, *, path=(), speed=9.0, heading=0.0,
          cooperative=True):
    return PeerIntent(
...
def test_end_to_end_follow_only_no_claim():
    ...
    r = resolve_conflicts(
        ...
        my_claim=None, obstacle_snapshots=[lead], peer_intents=[],
    )
...
def test_ignored_agent_never_reaches_the_corridor():
    ...
    r = resolve_conflicts(
        ...
        obstacle_snapshots=[adj], peer_intents=[],
    )
    assert r.diagnostics["tags"]["adj"] == IGNORE
    assert all(h >= _BIG for h in r.corridor.s_hi)


def test_cooperative_merge_peer_wins_ego_opens_gap():
    # peer merging in from the adjacent lane, committed earlier than ego
    path = [(25.0 + 0.9 * k, 3.4 - 0.16 * k) for k in range(20)]
    peer = _peer(2, (25.0, 3.4), committed_at_s=4.0, path=path, speed=9.0)
    r = resolve_conflicts(
        reference_samples=REF, ego_snapshot=EGO, my_actor_id=1,
        my_claim=_claim(committed_at_s=10.0),
        obstacle_snapshots=[], peer_intents=[peer],
    )
    assert r.diagnostics["roles"].get("2") == "make_gap"
    # ego is bounded behind the peer's projected station
    assert any(h < _BIG for h in r.corridor.s_hi)
    assert "2" in r.corridor.binding


def test_cooperative_merge_peer_loses_ego_proceeds():
    path = [(25.0 + 0.9 * k, 3.4 - 0.16 * k) for k in range(20)]
    peer = _peer(2, (25.0, 3.4), committed_at_s=20.0, path=path, speed=9.0)
    r = resolve_conflicts(
        reference_samples=REF, ego_snapshot=EGO, my_actor_id=1,
        my_claim=_claim(committed_at_s=10.0),
        obstacle_snapshots=[], peer_intents=[peer],
...
def test_latch_state_round_trips_and_holds():
    ...
    r1 = resolve_conflicts(
        reference_samples=REF, ego_snapshot=EGO, my_actor_id=1,
        my_claim=_claim(committed_at_s=10.0),
        peer_intents=[_peer(2, (25.0, 3.4), committed_at_s=11.0, path=path)],
    )
    assert r1.diagnostics["roles"].get("2") == "proceed"
    # peer re-commits slightly earlier (non-decisive) -> latch holds proceed
    r2 = resolve_conflicts(
        reference_samples=REF, ego_snapshot=EGO, my_actor_id=1,
        my_claim=_claim(committed_at_s=10.0),
        peer_intents=[_peer(2, (25.0, 3.4), committed_at_s=9.8, path=path)],
