...
class _DummyWaypoint:
    def __init__(
        ...
        *,
        ...
        self.ad_lane_id = 1000 + abs(int(road_id) * 10 + int(lane_id))
        self.parametric_offset = float(y_m)
        self.position = {"x": float(x_m), "y": float(y_m), "z": 0.0}
        self.heading = math.radians(float(yaw_deg))
...
class _NearestWaypointMap:
    ...
    def get_waypoint(self, location):
        if len(self._waypoints) == 0:
            return None
        x_m = float(location.get("x", 0.0)) if isinstance(location, dict) else float(location.x)
        y_m = float(location.get("y", 0.0)) if isinstance(location, dict) else float(location.y)
        return min(
            self._waypoints,
            key=lambda waypoint: (
                (float(waypoint.position["x"]) - x_m) ** 2
                + (float(waypoint.position["y"]) - y_m) ** 2
            ),
        )
...
class TrafficLightStopTests(unittest.TestCase):
    def test_find_stop_target_returns_last_non_junction_waypoint_before_junction(self):
        ...
        ego_position = dict(wp0.position)

        stop_target = find_stop_target_from_ego(
            global_planner=world_map,
    ...
    def test_find_stop_target_advances_to_next_junction_across_close_parallel_route_segments(self):
        ...
        for ego_x_m, ego_y_m in (
            ...
            _ = find_stop_target_from_ego(
                global_planner=world_map,
                ego_position=[float(ego_x_m), float(ego_y_m), 0.0],
    ...
    def test_behavior_planner_emits_stop_and_holds_commitment_until_green(self):
        ...
        self.assertEqual(first_result["decision"], "stop_at_intersection")
        ...
        self.assertEqual(second_result["decision"], "stop_at_intersection")
    ...
    def test_behavior_planner_releases_red_latch_for_green_signal_even_if_actor_changes(self):
        ...
        self.assertNotEqual(second_result["decision"], "stop_at_intersection")
    ...
    def test_find_relevant_signal_context_selects_signal_by_stop_waypoint_match(self):
        ...
        self.assertEqual(str(signal_context["signal_source"]), "actor_position_match")
    ...
    def test_find_relevant_signal_context_uses_lane_aware_stop_waypoint_match_for_later_signal(self):
        ...
        signal_context = find_relevant_signal_context(
            ...
            stop_target=stop_target,
            global_planner=_NearestWaypointMap(correct_far_wp, wrong_near_wp),
        )

        self.assertTrue(bool(signal_context["signal_found"]))
        self.assertEqual(int(signal_context["signal_actor_id"]), 301)
        self.assertEqual(str(signal_context["signal_state"]), "red")
        self.assertEqual(str(signal_context["signal_source"]), "custom_Global_Planner_lane_match")
    ...
    def test_behavior_planner_releases_stop_latch_when_ego_enters_junction(self):
        planner = RuleBasedBehaviorPlanner()
        stop_target = {
            "x_m": 0.0,
            "y_m": 12.0,
    ...
    def test_behavior_planner_debug_keeps_final_stop_state_while_red_latch_holds(self):
        ...
        self.assertEqual(first["decision"], "stop_at_intersection")
        ...
        self.assertEqual(second["decision"], "stop_at_intersection")
        self.assertTrue(bool(second["traffic_light_debug"]["should_stop_now"]))
        self.assertTrue(bool(second["traffic_light_debug"]["stop_decision_active"]))

    def test_behavior_planner_stops_for_red_even_when_stop_point_is_close(self):
        planner = RuleBasedBehaviorPlanner()
        result = planner.update(
            lane_safety_scores={1: 1.0},
            ego_lane_id=1,
            mode="NORMAL",
            traffic_signal_state="red",
            traffic_stop_target={"distance_m": 0.5, "lane_id": 1, "x_m": 0.0, "y_m": 0.5},
            ego_speed_mps=6.0,
            ego_max_deceleration_mps2=2.0,
            ego_in_junction=False,
        )
        ...
        self.assertTrue(bool(result["traffic_light_debug"]["should_stop_now"]))

    def test_stop_sign_wait_uses_wall_time_and_survives_cp_message_removal(self):
        stop_message = {
            "id": "stop_1",
            "type": "stop",
            "state": "stop",
            "stopping_point": [1.0, 0.0],
            "road_id": 1,
            "lane_id": 1,
        }
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            first = planner.update(
                ...
                ego_lane_id=1,
                mode="NORMAL",
                ego_position_xy=[1.1, 0.0],
                global_route_points=[[0.0, 0.0], [5.0, 0.0]],
                ego_speed_mps=0.9,
                current_time_s=1.0,
                wall_time_s=100.0,
            )
        ...
        self.assertEqual(first["decision"], "stop_sign")
        self.assertAlmostEqual(
            float(first["traffic_light_debug"]["stop_wait_remaining_s"]),
            3.0,
            places=3,
        )
        self.assertEqual(payload.get("control", []), [])
        self.assertEqual(second["decision"], "stop_sign")
        self.assertTrue(bool(second["stop"]))
        self.assertAlmostEqual(
            float(second["traffic_light_debug"]["stop_wait_elapsed_s"]),
            1.5,
            places=3,
        )
        self.assertAlmostEqual(
            float(second["traffic_light_debug"]["stop_wait_remaining_s"]),
            1.5,
            places=3,
        )
        self.assertEqual(third["decision"], "lane_follow")
        self.assertFalse(bool(third["stop"]))
        self.assertFalse(bool(third["traffic_light_debug"]["stop_latched"]))
        self.assertFalse(bool(third["traffic_light_debug"]["stop_decision_active"]))

    def test_stop_sign_wait_can_start_with_small_stop_target_mismatch_once_speed_is_low(self):
        stop_message = {
            "id": "stop_robust",
            "type": "stop",
            "state": "stop",
            "stopping_point": [3.0, 0.0],
            "road_id": 1,
            "lane_id": 1,
        }
        ...
        self.assertEqual(first["decision"], "stop_sign")
        self.assertTrue(bool(first["traffic_light_debug"]["stop_wait_started"]))
        self.assertAlmostEqual(
            float(first["traffic_light_debug"]["stop_wait_remaining_s"]),
            3.0,
            places=3,
        ...
        self.assertFalse(bool(second["stop"]))

    def test_cp_intersection_message_uses_traffic_light_stop_logic(self):
        intersection_message = {
            "id": "intersection_1",
            "type": "intersection",
            "state": "red",
            "stop": True,
            "stopping_point": [10.0, 0.0],
            "road_id": 1,
            "lane_id": 1,
        }
        ...
        self.assertEqual(first["decision"], "stop_at_intersection")
        self.assertFalse(bool(first["blue_dot_rolling"]))
        self.assertTrue(bool(first["stop"]))
        self.assertEqual(second["decision"], "stop_at_intersection")
        self.assertTrue(bool(second["traffic_light_debug"]["stop_latched"]))

    def test_cp_intersection_message_honors_explicit_stop_flag(self):
        intersection_message = {
            "id": "intersection_stop_flag",
            "type": "intersection",
            "state": "unknown",
            "stop": True,
            "stopping_point": [10.0, 0.0],
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            intersection_message["stop"] = False
            with open(message_path, "w", encoding="utf-8") as message_file:
                json.dump(
                    {
                        "schema_version": 1,
    ...
    def test_cp_traffic_light_stop_stays_latched_when_stop_point_becomes_slightly_behind(self):
    ...
    def test_cp_traffic_light_messages_follow_wall_time_polling(self):
        red_message = {
            "id": "tl_wall_time",
            "type": "traffic_light",
            "state": "red",
        ...
        green_message = dict(red_message)
        green_message["state"] = "green"
