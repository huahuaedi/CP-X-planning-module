...
class _FakeWaypoint:
    def __init__(self, x: float, y: float, *, road_id: int = 12, section_id: int = 3, lane_id: int = 1):
        ...
        self.ad_lane_id = 100 + abs(int(lane_id))
...
class _FakeWorldMap:
    def get_waypoint(self, location, project_to_road=True, lane_type=None):
        ...
        x = float(location.get("x", 0.0)) if isinstance(location, dict) else float(location.x)
        y = float(location.get("y", 0.0)) if isinstance(location, dict) else float(location.y)
        return _FakeWaypoint(x, y)
