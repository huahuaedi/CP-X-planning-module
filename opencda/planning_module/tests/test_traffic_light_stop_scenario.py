...
class _FakeWaypointWorldMap:
    def __init__(self, *, x: float, y: float, yaw_deg: float):
        self._position = {"x": float(x), "y": float(y), "z": 0.0}
        self._heading = math.radians(float(yaw_deg))

    def get_waypoint(self, location):
        del location
        return SimpleNamespace(position=self._position, heading=self._heading)
...
class TrafficLightStopScenarioTests(unittest.TestCase):
    ...
    def test_spawn_obstacles_uses_exact_marker_transform_resolution(self):
        ...
        spawned = spawn_obstacles(
            world=world,
            global_planner=_FakeWaypointWorldMap(x=20.0, y=50.0, yaw_deg=37.0),
            carla=_FakeCarla,
