...
class _FakeWaypointWorldMap:
    ...
    def get_waypoint(self, location, project_to_road=True, lane_type=None):
        ...
        waypoint = SimpleNamespace(
            ...
            ad_lane_id=100 + abs(int(self._lane_id)),
            parametric_offset=0.0,
            position={"x": float(self._waypoint_xy[0]), "y": float(self._waypoint_xy[1]), "z": 0.0},
            heading=0.0,
            lane_width_m=3.5,
...
class HighLevelRoutePlanningScenarioTests(unittest.TestCase):
    ...
    def test_workzone_message_is_inserted_once_when_ego_is_within_trigger_distance(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            runtime_state = initialize_runtime(
                ...
                global_planner=_FakeWaypointWorldMap([20.0, 0.0], road_id=20, section_id=0, lane_id=-2),
                carla=_FakeCarla,
            )
            ...
            route_summary, route_points, next_runtime_state = maybe_replan_global_route(
                ...
                sim_time_s=0.0,
                wall_time_s=0.0,
                ego_transform=_ego_transform(4.0, 0.0),
                active_global_route_points=[[0.0, 0.0], [10.0, 0.0], [20.0, 0.0], [30.0, 0.0]],
            )
            ...
            self.assertAlmostEqual(float(next_runtime_state.get("last_workzone_longitudinal_distance_m")), 16.0, places=3)

            with open(message_path, "r", encoding="utf-8") as message_file:
                payload = json.load(message_file)
            self.assertEqual(payload.get("lane_events", []), [])
            ...
            with open(message_path, "r", encoding="utf-8") as message_file:
                payload = json.load(message_file)
            lane_events = list(payload.get("lane_events", []))
            self.assertEqual(len(lane_events), 1)
            self.assertEqual(lane_events[0]["id"], "workzone_message")
            self.assertEqual(lane_events[0]["type"], "lane_closure")
            self.assertEqual(lane_events[0]["position"], [20.0, 0.0])
            ...
            _, _, final_runtime_state = maybe_replan_global_route(
                ...
                sim_time_s=0.0,
                wall_time_s=1000.0,
                ego_transform=_ego_transform(6.0, 0.0),
                active_global_route_points=[[0.0, 0.0], [10.0, 0.0], [20.0, 0.0], [30.0, 0.0]],
            )
            ...
            with open(message_path, "r", encoding="utf-8") as message_file:
                payload = json.load(message_file)
            self.assertEqual(len(list(payload.get("lane_events", []))), 1)
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with contextlib.redirect_stdout(captured_stdout):
                runtime_state = initialize_runtime(
                    ...
                    world=_FakeWorld(),
                    global_planner=_FakeWaypointWorldMap([1.0, 2.0]),
                    carla=_FakeCarla,
                )
    ...
    def test_initialize_runtime_does_not_fallback_to_actor_when_workzone_cube_is_missing(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            runtime_state = initialize_runtime(
                ...
                global_planner=_FakeWaypointWorldMap([13.0, 35.0], road_id=20, section_id=0, lane_id=-2),
                carla=_FakeCarla,
            )
    ...
    def test_workzone_is_retried_and_inserted_if_found_later(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            runtime_state = initialize_runtime(
                ...
                world=_FakeWorld(),
                global_planner=_FakeWaypointWorldMap([0.0, 0.0], road_id=20, section_id=0, lane_id=-2),
                carla=_FakeCarla,
            )
            ...
            _, _, next_runtime_state = maybe_replan_global_route(
                ...
                global_planner=_FakeWaypointWorldMap([41.0, 51.0], road_id=20, section_id=0, lane_id=-2),
                carla=_FakeCarla,
                sim_time_s=0.0,
                wall_time_s=0.0,
                ego_transform=_ego_transform(33.0, 41.0),
                active_global_route_points=[[0.0, 0.0], [41.0, 51.0]],
            )
            ...
            with open(message_path, "r", encoding="utf-8") as message_file:
                payload = json.load(message_file)
            lane_events = list(payload.get("lane_events", []))
            self.assertEqual(lane_events[0]["position"], [41.0, 51.0])
            self.assertEqual(lane_events[0]["road_id"], 20)
            self.assertEqual(lane_events[0]["section_id"], 0)
            self.assertEqual(lane_events[0]["lane_id"], 1)
            self.assertEqual(lane_events[0]["opendrive_lane_id"], -2)
    ...
    def test_workzone_does_not_inject_virtual_obstacle_before_or_after_message(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            snapshots_before, next_runtime_state = filter_dynamic_obstacle_snapshots(
                ...
                global_planner=_FakeWaypointWorldMap([13.0, 35.0]),
                carla=_FakeCarla,
                object_snapshots=[],
                sim_time_s=0.0,
                wall_time_s=0.0,
    ...
    def test_cooperative_message_trigger_uses_route_longitudinal_distance_not_time(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            message_path = os.path.join(tmp_dir, "cp_message.json")
            with open(message_path, "w", encoding="utf-8") as message_file:
                json.dump([], message_file)
            ...
            runtime_state = initialize_runtime(
                scenario_cfg={
                    "runtime": {
                        "workzone_name": "workzone",
                        "cooperative_message_trigger_longitudinal_distance_m": 15.0,
                        "cp_message_id": "workzone_distance_message",
                        "cp_message_path": message_path,
                    }
                },
                world=_FakeWorld(
