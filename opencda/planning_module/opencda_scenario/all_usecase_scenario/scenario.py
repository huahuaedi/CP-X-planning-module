...
def _closest_driving_waypoint(global_planner, transform: Any):
    location = getattr(transform, "location", None)
    if location is None or global_planner is None:
        return None
    try:
        return global_planner.get_waypoint(world_point(location))
    except Exception:
        return None


def _record_marker(*, global_planner, carla, marker_obj: Any, prefix: str = "") -> Dict[str, object]:
    transform = _object_transform(marker_obj)
    waypoint = _closest_driving_waypoint(global_planner, transform)
    ...
    if waypoint_location is not None:
        waypoint_transform = carla.Transform(
            carla.Location(
                x=float(waypoint_location["x"]),
                y=float(waypoint_location["y"]),
                z=float(waypoint_location["z"]),
            ),
...
def _records_by_prefix(*, world, global_planner=None, carla, prefix: str, **_) -> List[Dict[str, object]]:
    return [
        _record_marker(global_planner=global_planner, carla=carla, marker_obj=obj, prefix=prefix)
        for obj in _find_markers_by_prefix(world, carla, prefix)
    ]


def _records_by_indexed_names(*, world, global_planner=None, carla, prefix: str, **_) -> List[Dict[str, object]]:
...
def _record_by_name(*, world, global_planner=None, carla, marker_name: str, **_) -> Dict[str, object] | None:
    marker_obj = _find_marker(world, carla, marker_name)
    if marker_obj is None:
        return None
    return _record_marker(global_planner=global_planner, carla=carla, marker_obj=marker_obj)
...
def _intersection_signal_state_for_marker(
    *,
    ...
    del preferred_actor_name, stop_waypoint_match_distance_m
    ...
    for actor in traffic_light_actors:
        ...
        priority = (0.0, float(actor_distance_m), str(actor_name).strip().lower())
...
def initialize_runtime(
    ...
    **extras,
    ...
    hazard_marker = _record_by_name(
        world=world,
        global_planner=extras.get("global_planner", None),
        carla=carla,
