"""Standalone Town10 reroute test using the custom Global_Planner."""
...
def _waypoint_key(waypoint: Any) -> Tuple[int, int, int, float] | None:
    ...
    return (
        ...
        int(getattr(waypoint, "ad_lane_id", 0)),
        round(float(getattr(waypoint, "parametric_offset", 0.0)), 6),
    )
...
def _resolve_marker_waypoint_state(
    *,
    ...
    waypoint = planner.get_waypoint(world_point(location))
    if waypoint is None:
        return None
    ...
    return MarkerWaypointState(
        ...
        opendrive_lane_id=int(getattr(waypoint, "lane_id", 0)),
        waypoint_key=key,
        waypoint=waypoint,
    )
...
def _blocked_segments_for_close_waypoint(
    ...
    if close_waypoint is None:
        return []
    return [("ad_lane", int(close_waypoint.ad_lane_id))]
...
def _plan_initial_route(
    *,
    ...
    planner.add_blocked_lane(int(close_state.waypoint.ad_lane_id))
    blocked_lanes = [f"ad_lane:{int(close_state.waypoint.ad_lane_id)}"]
    try:
        route = planner.trace_route(
            {
                "x": float(start_state.waypoint_position_xy[0]),
                "y": float(start_state.waypoint_position_xy[1]),
                "z": 0.0,
            },
            {
                "x": float(end_state.waypoint_position_xy[0]),
                "y": float(end_state.waypoint_position_xy[1]),
                "z": 0.0,
            },
            activate_route=True,
        )
        summary = route_summary(planner, route, position=start_state.waypoint_position_xy)
        print("new path generated by custom Global_Planner")
        return summary, blocked_lanes, "custom_Global_Planner_blocked_lane"
    except Exception as exc:
        summary = route_summary(planner, None, debug_reason=str(exc))
        return summary, blocked_lanes, "custom_Global_Planner_blocked_lane"
...
def _build_planner(world, scenario_cfg, sample_distance_m: float) -> Tuple[Any, GlobalPlanner]:
    world_map = world.get_map()
    print(
        "[REROUTE_TEST] loading custom Global_Planner for "
        f"{getattr(world_map, 'name', '<unknown>')} with sample_distance_m={float(sample_distance_m):.2f}"
    )
    planner = build_custom_global_planner(
    ...
    try:
        while True:
            ...
            if close_state is None:
                ...
                continue
            if topdown_camera is not None:
                close_waypoint_position = waypoint_position(close_state.waypoint) or {"z": 0.0}
                topdown_camera.set_transform(
                    _topdown_camera_transform_from_target(
                        ...
                        z_m=float(close_waypoint_position.get("z", 0.0)),
