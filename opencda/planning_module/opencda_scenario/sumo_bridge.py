...
class OpenCDASumoBridge:
    ...
    def __init__(
        ...
        *,
        ...
        for traffic_light in list(self.world.get_actors()):
            if str(getattr(traffic_light, "type_id", "")) != "traffic.traffic_light":
                continue
            get_opendrive_id = getattr(traffic_light, "get_opendrive_id", None)
            if not callable(get_opendrive_id):
                continue
            try:
                landmark_id = str(get_opendrive_id() or "").strip()
            except Exception:
                continue
            if not landmark_id:
                continue
            self._tls[landmark_id] = traffic_light
