...
def run_loaded_world(client, world, scenario_cfg: Mapping[str, object], carla) -> int:
    """Standalone driver. External benchmarks consume iter_planning_steps directly."""
    steps = iter_planning_steps(client, world, scenario_cfg, carla)
    try:
        for result in steps:
            result["vehicle"].apply_control(result["control"])
    finally:
        steps.close()
    return 0


def iter_planning_steps(client, world, scenario_cfg, carla, external=None):
    """Yield controls with persistent state; external callers own actors and ticks."""
    ...
    if external is not None:
        mpc_cfg = external.configure_mpc(mpc_cfg)
    ...
    if external is not None:
        ego_vehicle = external.vehicle
        aligned_spawn_transform = ego_vehicle.get_transform()
        initial_global_route_summary = external.install_route(global_planner, world_map, carla)
        initial_route_points = list(initial_global_route_summary.route_waypoints)
        aligned_destination_transform = external.destination_transform
        global_route_start_location = external.route[0][0].location
        global_route_goal_location = aligned_destination_transform.location
        topdown_focus_points = initial_route_points
    ...
    camera_blueprint = None if external is not None else _camera_blueprint(
        world=world,
    ...
    try:
        ...
        while True:
            ...
            if math.hypot(
                ...
                yield {"vehicle": ego_vehicle, "control": carla.VehicleControl(brake=1.0),
                       "done": True, "trajectory": [], "status": mpc.get_runtime_status()}
                print("Reached final destination.")
                return 0

            sim_time_s = float(world.get_snapshot().timestamp.elapsed_seconds)
            # Benchmark decisions must not depend on how fast workers execute.
            wall_time_s = sim_time_s if external is not None else float(time.perf_counter())

            dynamic_object_snapshots: List[dict] = []
            if external is not None:
                dynamic_object_snapshots = external.obstacles()
            elif bool(use_cp_obstacle_pipeline):
                ...
                if bool(traffic_light_stop_enabled):
                    ...
                    if not bool(tick_ego_in_junction):
                        tick_traffic_stop_target = find_stop_target_from_ego(
                            ...
                            query_key="ego" if external is None else external.signal_query_key,
                        )
            ...
            if str(current_applied_behavior) == "reroute":
                force_behavior_replan = True
            if external is not None:
                cached_control_step_idx = external.control_index(sim_time_s, mpc.dt_s)
            ...
            if mpc.should_replan(sim_time_s) or bool(force_behavior_replan):
                ...
                if str(current_applied_behavior) == "reroute" and external is not None:
                    # A benchmark route is mandatory: do not replace it with a shortcut.
                    current_applied_behavior = "lane_follow"
                if str(current_applied_behavior) == "reroute":
                ...
                # External mode uses distance-based caps; this current-speed cap
                # would otherwise prevent a stationary car from starting.
                if (
                    external is None
                    and bool(is_fixed_stop_decision(current_applied_behavior))
                ...
                if external is not None:
                    external.plan_time_s = float(sim_time_s)
            ...
            if not planned_trajectory:
                ...
                control = carla.VehicleControl(brake=0.3)
            else:
                if external is not None:
                    cached_control_step_idx = external.control_index(sim_time_s, mpc.dt_s)
                control_step_idx = min(int(cached_control_step_idx), len(cached_control_sequence) - 1)
                ...
                control = _control_from_mpc(mpc, carla, current_acceleration_mps2, current_steering_rad)
                if external is not None:
                    control = external.track_control(control, planned_trajectory, control_step_idx, mpc.dt_s, sim_time_s)
                if external is None and cached_control_step_idx < len(cached_control_sequence) - 1:
                    cached_control_step_idx += 1
            yield {
                "vehicle": ego_vehicle, "control": control, "done": False,
                "trajectory": list(planned_trajectory), "status": mpc.get_runtime_status(),
                "behavior": current_applied_behavior, "fsm": cached_planner_lc_state,
                "obstacle_count": len(dynamic_object_snapshots),
                "control_index": int(cached_control_step_idx),
                "traffic_light": dict(traffic_light_debug),
                "temporary_destination": list(temporary_destination_state or []),
            }
        ...
        try:
            metric_artifacts = write_planning_metrics_artifacts(
                ...
                collision_events_available=external is None,
            )
            print(
