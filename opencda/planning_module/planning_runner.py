...
def _align_transform_to_lane(global_planner, carla, transform):
    waypoint = global_planner.get_waypoint(world_point(transform.location))
    if waypoint is None:
        return transform, None
    position = waypoint_position(waypoint)
    if position is None:
        return transform, None
    aligned_transform = carla.Transform(
        carla.Location(x=position["x"], y=position["y"], z=position["z"]),
        carla.Rotation(yaw=math.degrees(float(waypoint.heading or 0.0))),
    )
    return aligned_transform, waypoint
...
def _spawn_vehicle(
    ...
    *,
    ...
    if anchor_transform is not None and lane_transform is not None:
        ...
        # The anchor rotation is only used when no custom Global_Planner lane
        # direction was available. Every normal ego spawn keeps the lane yaw.
...
def _spawn_collision_sensor(world, carla, ego_vehicle, metrics_recorder: EvaluationMetricsRecorder):
    ...
    def _on_collision(event) -> None:
        ...
        metrics_recorder.record_collision(event_id=f"{frame_id}:{other_actor_id}")
...
def _lane_center_reference_points_for_visualization(
    reference_samples: Sequence[Mapping[str, object]] | None,
    *,
    enabled: bool,
) -> List[List[float]] | None:
    if not bool(enabled) or reference_samples is None:
        return None
    ...
    for sample in reference_samples:
        if not isinstance(sample, Mapping):
            continue
        try:
            x_ref_m = float(sample.get("x_ref_m"))
            y_ref_m = float(sample.get("y_ref_m"))
        except (TypeError, ValueError):
            continue
        if math.isfinite(x_ref_m) and math.isfinite(y_ref_m):
            reference_points.append([x_ref_m, y_ref_m])
...
def _draw_planning_overlay(
    *,
    ...
    lane_center_reference_points: Sequence[Sequence[float]] | None,
    temporary_destination_state: Sequence[float] | None,
    ...
    if lane_center_reference_points is not None:
        for point_xy in lane_center_reference_points:
            if len(point_xy) < 2:
                continue
            ...
            if pixel is not None:
                pygame.draw.circle(surface, (70, 255, 90), pixel, 4)
                pygame.draw.circle(surface, (20, 20, 20), pixel, 4, width=1)
...
def _draw_hud_lines(surface, font, lines: Sequence[str], top_right_margin_px: tuple[int, int]) -> None:
    if pygame is None or font is None or len(lines) == 0:
        return

    right_margin_px, y0_px = int(top_right_margin_px[0]), int(top_right_margin_px[1])
    ...
    x0_px = max(0, int(surface.get_width()) - int(right_margin_px) - int(box_width_px))
    ...
    if left_image is not None:
        ...
        if isinstance(topdown_overlay, Mapping):
            _draw_planning_overlay(
                ...
                lane_center_reference_points=topdown_overlay.get("lane_center_reference_points", None),
...
def _replan_route_around_static_intersection_obstacle(
    *,
    ...
    blocked_waypoint = global_planner.get_waypoint(blocked_point)
    if blocked_waypoint is None:
        return None, []
    global_planner.add_blocked_lane(int(blocked_waypoint.ad_lane_id))
    previous_route = global_planner.active_route
    try:
        replacement_route = global_planner.trace_route(
            {"x": start_xy[0], "y": start_xy[1], "z": 0.0},
            {"x": goal_xy[0], "y": goal_xy[1], "z": float(getattr(goal_location, "z", 0.0))},
            activate_route=True,
        )
    except Exception:
        global_planner.active_route = previous_route
        return None, []
    route_summary_value = route_summary(global_planner, replacement_route, position=start_xy)
    route_points = list(route_summary_value.route_waypoints)
    if len(route_points) == 0:
        return None, []
    return route_summary_value, route_points
...
def _stop_target_state_from_behavior_output(
    *,
    ...
    try:
        ...
        stop_waypoint = global_planner.get_waypoint(
            {"x": stop_x_m, "y": stop_y_m, "z": ego_z_m}
        )
        if stop_waypoint is not None:
            stop_heading_rad = float(stop_waypoint.heading or stop_heading_rad)
        return [
            float(stop_x_m),
            float(stop_y_m),
            0.0,
...
def _follow_target_state_from_behavior_output(
    *,
    ...
    try:
        ...
        ego_z_m = float(getattr(ego_transform.location, "z", 0.0))
        follow_waypoint = global_planner.get_waypoint(
            {"x": follow_x_m, "y": follow_y_m, "z": ego_z_m}
        )
        if follow_waypoint is not None:
            follow_position = waypoint_position(follow_waypoint) or {"x": follow_x_m, "y": follow_y_m}
            follow_x_m = float(follow_position["x"])
            follow_y_m = float(follow_position["y"])
            follow_heading_rad = float(follow_waypoint.heading or follow_heading_rad)
            follow_lane_id = float(behavior_lane_id(follow_waypoint))
...
def _collect_environment_obstacle_snapshots(world, global_planner, carla, obstacle_prefix: str) -> List[dict]:
    snapshots: List[dict] = []
    ...
    for env_obj in _find_environment_objects_by_prefix(world, carla, obstacle_prefix):
        transform = env_obj.transform
        location = transform.location
        nearest_waypoint = global_planner.get_waypoint(world_point(location))
        heading_rad = float(math.radians(transform.rotation.yaw))
        if nearest_waypoint is not None:
            heading_rad = float(nearest_waypoint.heading or heading_rad)
...
def run_loaded_world(client, world, scenario_cfg: Mapping[str, object], carla) -> int:
    ...
    lane_center_reference_visualization_enabled = bool(
        visualization_cfg.get("visualize_lane_center_ref", False)
    )

    sample_distance_m = float(planning_cfg.get("waypoint_sample_distance_m", 2.0))
    global_planner = build_custom_global_planner(
        scenario_cfg=scenario_cfg,
    ...
    initial_route = global_planner.trace_route(
        world_point(global_route_start_location),
        world_point(global_route_goal_location),
        sampling_resolution_m=float(sample_distance_m),
        activate_route=True,
    )
    initial_global_route_summary = route_summary(
        global_planner,
        initial_route,
        position=global_route_start_location,
    )
    lane_center_waypoints, road_cfg = build_lane_center_waypoints_from_route(initial_route)
    ...
    initial_lane_context = lane_context(global_planner, aligned_spawn_transform.location)
    ...
    local_lane_center_reference: List[Dict[str, float]] = []
    ...
    try:
        ...
        # Intersection connectors can have unique road IDs and no lane siblings.
        # We freeze
        ...
        while True:
            ...
            if not bool(use_cp_obstacle_pipeline):
                ...
                if bool(traffic_light_stop_enabled):
                    ...
                    tick_ego_waypoint = global_planner.get_waypoint(world_point(tick_ego_transform.location))
                    ...
                    if not bool(tick_ego_in_junction):
                        tick_traffic_stop_target = find_stop_target_from_ego(
                            global_planner=global_planner,
                            ego_position=world_point(tick_ego_transform.location),
            ...
            if bool(is_fixed_stop_decision(current_applied_behavior)):
                if float(ego_state[2]) <= 0.25:
                    force_behavior_replan = True
            elif _should_force_stationary_release_replan(
            ...
            if mpc.should_replan(sim_time_s) or bool(force_behavior_replan):
                ...
                ego_waypoint = global_planner.get_waypoint(world_point(ego_transform.location))
                ...
                current_route_summary = route_summary(global_planner, position=ego_state)
                current_lane_context = lane_context(global_planner, ego_state)
                ...
                if ego_in_junction and int(_pre_junction_lane_id) != 0:
                    # Intersection connectors can have unique road IDs and no lane
                    # siblings, regardless of the approach lane.
                ...
                for obs in predicted_snapshots:
                    ...
                    obs_ctx = lane_context(global_planner, obs)
                ...
                ego_offset_info = compute_ego_lane_offset(
                    global_planner=global_planner,
                    ego_position=world_point(ego_transform.location),
                ...
                if _prev_dest_xy is not None:
                    planning_temporary_route_summary = route_summary(global_planner, position=_prev_dest_xy)
                ...
                if str(current_applied_behavior) == "reroute":
                    print("[BEHAVIOR] executing reroute request via custom Global_Planner.")
                    reroute_messages = (
                    ...
                    reroute_result = reroute_from_lane_closure_messages(
                        ...
                        global_planner=global_planner,
                        ego_position=world_point(ego_transform.location),
                        goal_position=world_point(global_route_goal_location),
                    ...
                    if rerouted_route_summary is not None and len(rerouted_route_points) > 0:
                        ...
                        last_world_debug_route_draw_time_s = -float("inf")
                        replacement_route = reroute_result.get("route", None)
                        if replacement_route is not None:
                            lane_center_waypoints, replacement_road_cfg = build_lane_center_waypoints_from_route(replacement_route)
                            road_cfg.update(replacement_road_cfg)
                        current_route_summary = route_summary(global_planner, position=ego_state)
                        planning_temporary_route_summary = current_route_summary
                        _prev_dest_xy = None
                        _prev_mode = None
                        ...
                        reroute_failed = True
                        current_applied_behavior = "emergency_brake"
                        print(
                            "[BEHAVIOR] custom Global_Planner could not find a replacement route; "
                            "the ego vehicle will stop and the closure message remains pending."
                        )
                motion_behavior_decision = (
                    "lane_follow"
                    if str(current_applied_behavior) == "reroute" and not bool(reroute_failed)
                    else str(current_applied_behavior)
                )
                ...
                # ---- Temporary destination via custom Global_Planner waypoints ---- #
                ...
                if temporary_destination_state is not None and len(temporary_destination_state) >= 2:
                    temporary_route_summary = route_summary(
                        global_planner,
                        position=temporary_destination_state,
                    )
            ...
            if display is not None and topdown_queue is not None and chase_queue is not None:
                ...
                hud_lines = [
                    ...
                    "Overlay: yellow=route  green=lane_ref  orange=obstacle  red=risky  blue=target",
                ]
                ...
                if topdown_camera is not None:
                    ...
                    topdown_overlay = {
                        ...
                        "lane_center_reference_points": _lane_center_reference_points_for_visualization(
                            local_lane_center_reference,
                            enabled=bool(lane_center_reference_visualization_enabled),
                        ),
