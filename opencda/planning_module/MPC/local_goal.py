...
def _lane_count_from_inputs(
    ...
    lane_ids = _available_lane_ids_from_waypoints(lane_center_waypoints)
    return len(lane_ids) if len(lane_ids) > 0 else 1


def _available_lane_ids_from_waypoints(
    lane_center_waypoints: Sequence[Mapping[str, object]],
) -> List[int]:
    lane_ids: List[int] = []
    for item in lane_center_waypoints:
        raw_lane_id = item.get("lane_id", None)
        if raw_lane_id is None:
            continue
        lane_id = int(raw_lane_id)
        if lane_id == int(INVALID_LANE_ID) or lane_id in lane_ids:
            continue
        lane_ids.append(int(lane_id))
    return lane_ids


def _clamp_lane_id_to_available(
    ...
    if len(normalized_lane_ids) == 0:
        return int(raw_lane_id or INVALID_LANE_ID)
    lane_id = int(raw_lane_id if raw_lane_id is not None else normalized_lane_ids[0])
    if lane_id in normalized_lane_ids:
        return int(lane_id)
    return int(
        min(
            normalized_lane_ids,
...
def _waypoint_map(
    ...
    waypoint_by_xy: Dict[tuple[float, float], Mapping[str, object]] = {}
    for waypoint in waypoints:
        position = _position_of_waypoint(waypoint)
        if position is None:
            continue
        waypoint_by_xy[_waypoint_key(float(position[0]), float(position[1]))] = waypoint
    return waypoint_by_xy
...
def _nearest_waypoint_on_lane(
    ...
    valid_waypoints = [
        waypoint
        for waypoint in lane_center_waypoints
        if int(waypoint.get("lane_id", INVALID_LANE_ID)) == int(target_lane_id)
    ...
    if current_carla_waypoint is not None and hasattr(current_carla_waypoint, "next"):
        ...
        if carla_follow_result is not None:
            ...
            if snapped_state is not None:
                snapped_state.append(float(int(snap_lane_id)))
...
def build_destination_on_lane(
    ...
    del road_cfg
    clamped_lane_id = _clamp_lane_id_to_available(
        raw_lane_id=int(target_lane_id),
        available_lane_ids=_available_lane_ids_from_waypoints(lane_center_waypoints),
    )
...
def build_lane_center_reference_to_destination(
    ...
    if destination_lane_id == int(INVALID_LANE_ID):
    ...
    destination_lane_id = _clamp_lane_id_to_available(
        raw_lane_id=int(destination_lane_id),
...
def compute_temporary_destination_state_from_route(
    ...
    if final_destination_state is not None and len(final_destination_state) >= 4:
        ...
        if final_distance_m <= float(local_goal_cfg.get("lock_to_final_distance_m", 30.0)):
            ...
            if len(final_state) < 5:
                final_state.append(float(int(target_lane_id or INVALID_LANE_ID)))
            return final_state

    inferred_lane_id = int(target_lane_id or INVALID_LANE_ID)
    if lane_center_waypoints is not None and len(lane_center_waypoints) > 0:
        ...
        if snapped_waypoint is not None:
            ...
            inferred_lane_id = int(snapped_waypoint.get("lane_id", inferred_lane_id))
...
def compute_temporary_destination_state(
    ...
    if len(destination_state) >= 5:
        destination_state[4] = float(int(destination_state[4]))
    else:
