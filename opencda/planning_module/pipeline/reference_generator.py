...
class ReferenceGenerator:
    ...
    def __init__(
        ...
        *,
        ...
        mpc: Any,
        map_planner: Any,
        map_waypoint_from_location: Callable[[carla.Location], Any],
        lane_id_at_location: Callable[[carla.Location], int],
        body_frame_xy: Callable[..., tuple[float, float]],
        target_speed_mps: float,
        lookahead_m: float,
        drivable_waypoint_from_location: (
            Callable[[carla.Location], Any] | None
        ) = None,
    ) -> None:
    ...
    def map_waypoint(self, location: carla.Location):
        ...
        *,
        ...
        for longitudinal_m, lateral_m in sorted(points):
            ...
            try:
                ...
                lane_heading_rad = math.radians(
                    float(transform.rotation.yaw)
                )
                lane_width_m = float(
                    getattr(waypoint, "lane_width", 0.0) or 0.0
                )
            except Exception:
                return DrivableFootprintOccupancy(
                    valid=False,
            ...
            dx_m = float(point_x_m) - float(center.x)
            dy_m = float(point_y_m) - float(center.y)
            point_lateral_m = (
    ...
    def _lane_corridor_geometry(
        ...
        *,
        ...
        try:
            ...
            transform = waypoint.transform
            center = transform.location
            lane_heading_rad = math.radians(float(transform.rotation.yaw))
                    ...
                    math.radians(
                        float(
                            getattr(
                                ...
                                "yaw",
                                math.degrees(previous_heading_rad),
                            )
                        )
                    )
