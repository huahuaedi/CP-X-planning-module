...
class StableLaneIdTracker:
    ...
    def update(self, waypoint, *, on_discontinuity=None) -> int:
        ...
        if self._waypoint is not None and self._lane_id is not None:
            ...
            try:
                previous_location = self._waypoint.transform.location
                new_location = waypoint.transform.location
                discontinuity_distance_m = math.hypot(
                    float(new_location.x) - float(previous_location.x),
                    float(new_location.y) - float(previous_location.y),
                )
            except Exception:
