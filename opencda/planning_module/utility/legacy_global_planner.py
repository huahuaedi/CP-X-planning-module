...
class AStarGlobalPlanner:
    ...
    def load(self, force_rebuild: bool = False) -> None:
        """Compatibility no-op for the custom planner adapter interface."""
        del force_rebuild
    ...
    @staticmethod
    def _point_xy(point: Mapping[str, object] | Sequence[object]) -> Tuple[float, float]:
        if isinstance(point, Mapping):
            return (
                float(point.get("x", point.get("x_m", 0.0))),
                float(point.get("y", point.get("y_m", 0.0))),
            )
        return (
            float(point[0]) if len(point) >= 1 else 0.0,
            float(point[1]) if len(point) >= 2 else 0.0,
        )
    ...
    def get_waypoint(
        ...
        """Return the nearest CARLA waypoint for compatibility with the runner."""
        x_m, y_m = self._point_xy(position)
        nearest_query = self.nearest_waypoint_query(x_m=float(x_m), y_m=float(y_m))
        if nearest_query is None:
            return None
        if search_radius_m is not None and float(nearest_query.distance_m) > float(search_radius_m):
            return None
        nearest_node = self._nodes[int(nearest_query.index)]
        return nearest_node.carla_waypoint
    ...
    def trace_route(
        ...
        *,
        ...
        del replace_stored_route
        return self.plan_route(start_xy=self._point_xy(start), goal_xy=self._point_xy(goal))

    def register_imported_route(
        self,
        route_points: Sequence[Mapping[str, object] | Sequence[object]],
    ) -> RoutePlanSummary | None:
        normalized_points = [self._point_xy(point) for point in list(route_points or [])]
        if len(normalized_points) < 2:
            return None
        summary = self.plan_route(
            start_xy=normalized_points[0],
            goal_xy=normalized_points[-1],
        )
        if not bool(getattr(summary, "route_found", False)):
            return None
        summary.route_waypoints = [[float(x_m), float(y_m)] for x_m, y_m in normalized_points]
        self.store_initial_route(
            summary=summary,
            per_waypoint_options=["LANEFOLLOW"] * len(normalized_points),
            per_waypoint_lane_ids=[int(summary.optimal_lane_id)] * len(normalized_points),
        )
        return summary
    ...
    def block_lane_at_position(
        ...
        if nearest_query is None:
            return None
        lane_id = int(nearest_query.lane_id)
    ...
    def segment_keys_for_raw_carla_lane(
        ...
        *,
        ...
        def _matching_segment_keys(*, ignore_section: bool) -> List[Tuple[str, int]]:
            segment_keys: List[Tuple[str, int]] = []
            for node in self._nodes:
                ...
                if requested_road_numeric is not None and int(getattr(carla_waypoint, "road_id", 0)) != int(requested_road_numeric):
                    continue
                if (
                    not bool(ignore_section)
                    and requested_section_id is not None
                    and int(getattr(carla_waypoint, "section_id", 0)) != int(requested_section_id)
                ):
                    continue
        ...
        normalized_direction = str(direction).strip().lower() if direction is not None else ""
        segment_keys = _matching_segment_keys(ignore_section=False)
        if len(segment_keys) == 0 and requested_section_id is not None:
            segment_keys = _matching_segment_keys(ignore_section=True)
        return segment_keys
    ...
    def _set_road_edges_inf(
        ...
        *,
        ...
        def _block_matching_edges(*, ignore_section: bool) -> int:
            blocked_edge_count = 0
            for section_id, section_edges in dict(sections or {}).items():
                if (
                    not bool(ignore_section)
                    and raw_section_id is not None
                    and int(section_id) != int(raw_section_id)
                ):
                    continue
        ...
        blocked_edge_count = _block_matching_edges(ignore_section=False)
        if blocked_edge_count <= 0 and raw_section_id is not None:
            blocked_edge_count = _block_matching_edges(ignore_section=True)
        return int(blocked_edge_count)

    @staticmethod
    def _set_carla_graph_edge_inf(graph, *, edge_start: int, edge_end: int) -> bool:
        if graph is None or not hasattr(graph, "has_edge") or not graph.has_edge(edge_start, edge_end):
            return False
        graph[edge_start][edge_end]["length"] = float("inf")
        if "weight" in graph[edge_start][edge_end]:
            graph[edge_start][edge_end]["weight"] = float("inf")
    ...
    def _carla_graph_edge_for_waypoint(self, carla_waypoint) -> Tuple[int, int] | None:
        if self._carla_route_planner is None or carla_waypoint is None:
            return None

        road_id_to_edge = getattr(self._carla_route_planner, "_road_id_to_edge", None)
        road_id = int(getattr(carla_waypoint, "road_id", 0))
        section_id = int(getattr(carla_waypoint, "section_id", 0))
        lane_id = int(getattr(carla_waypoint, "lane_id", 0))

        if road_id_to_edge is not None:
            sections = dict(road_id_to_edge.get(int(road_id), {}) or {})
            candidate_edge = dict(sections.get(int(section_id), {}) or {}).get(int(lane_id))
            if isinstance(candidate_edge, (list, tuple)) and len(candidate_edge) >= 2:
                return int(candidate_edge[0]), int(candidate_edge[1])
            for section_edges in sections.values():
                candidate_edge = dict(section_edges or {}).get(int(lane_id))
                if isinstance(candidate_edge, (list, tuple)) and len(candidate_edge) >= 2:
                    return int(candidate_edge[0]), int(candidate_edge[1])

        localize = getattr(self._carla_route_planner, "_localize", None)
        location = getattr(getattr(carla_waypoint, "transform", None), "location", None)
        if callable(localize) and location is not None:
            try:
                candidate_edge = localize(location)
            except Exception:
                candidate_edge = None
            if isinstance(candidate_edge, (list, tuple)) and len(candidate_edge) >= 2:
                return int(candidate_edge[0]), int(candidate_edge[1])
    ...
    def blocked_carla_graph_edges_for_waypoints(
        self,
        blocked_waypoints: Sequence[object] | None,
    ) -> List[Tuple[int, int]]:
        blocked_edges: List[Tuple[int, int]] = []
        seen_edges: set[Tuple[int, int]] = set()
        for blocked_waypoint in list(blocked_waypoints or []):
            blocked_edge = self._carla_graph_edge_for_waypoint(blocked_waypoint)
            if blocked_edge is None or blocked_edge in seen_edges:
                continue
            seen_edges.add(blocked_edge)
            blocked_edges.append((int(blocked_edge[0]), int(blocked_edge[1])))
    ...
    def _location_xy(location) -> Tuple[float, float] | None:
        ...
        if isinstance(location, Mapping):
            try:
                return (
                    float(location.get("x", location.get("x_m"))),
                    float(location.get("y", location.get("y_m"))),
                )
    ...
    def plan_route_from_locations_with_blocked_carla_waypoints(
        self,
        *,
        start_location,
        goal_location,
        ...
        blocked_edges = self.blocked_carla_graph_edges_for_waypoints(blocked_waypoints)
        if len(blocked_edges) == 0:
            ...
            if not bool(getattr(route_summary, "route_found", False)):
                setattr(
                    route_summary,
                    "debug_reason",
                    "No CARLA route-planner edge matched the blocked waypoint.",
                )
            return route_summary

        modified_graph = graph.copy() if hasattr(graph, "copy") else copy.deepcopy(graph)
        blocked_edge_count = 0
        for edge_start, edge_end in blocked_edges:
            blocked_edge_count += int(
                self._set_carla_graph_edge_inf(
                    modified_graph,
        ...
        original_graph = self._carla_route_planner._graph
        self._carla_route_planner._graph = modified_graph
        try:
            route_summary = self.plan_route_from_locations(
                start_location=start_location,
                goal_location=goal_location,
            ...
            self._carla_route_planner._graph = original_graph

        if blocked_edge_count <= 0 and not bool(getattr(route_summary, "route_found", False)):
            setattr(
                route_summary,
                "debug_reason",
                "No CARLA route-planner graph edge could be marked unavailable for the blocked waypoint.",
            )
        return route_summary
    ...
    def _compute_local_lane_context(
        ...
        if runtime_waypoint is not None:
            ...
            return {
                ...
                "heading_rad": math.radians(float(getattr(runtime_waypoint.transform.rotation, "yaw", 0.0))),
                "is_intersection": bool(getattr(runtime_waypoint, "is_junction", False)),
                "lane_width_m": max(0.1, float(getattr(runtime_waypoint, "lane_width", 3.5))),
            }
        ...
        return {
            ...
            "heading_rad": float(nearest_node.heading_rad),
            "is_intersection": bool(nearest_node.is_intersection),
            "lane_width_m": max(0.1, float(nearest_node.lane_width_m)),
        }
