...
from utility import waypoint_position, world_point
...
def _spawn_static_vehicle_at_marker(
    *,
    ...
    if global_planner is not None:
        nearest_waypoint = global_planner.get_waypoint(world_point(marker_transform.location))
        waypoint_location = waypoint_position(nearest_waypoint)
        if waypoint_location is not None:
            waypoint_transform = carla.Transform(
                ...
                carla.Rotation(yaw=math.degrees(float(nearest_waypoint.heading or 0.0))),
            )
    ...
    *,
    ...
    for marker_name in marker_names:
        ...
        nearest_waypoint = (
            ...
            else global_planner.get_waypoint(world_point(marker.transform.location))
        )
        waypoint_location = waypoint_position(nearest_waypoint)
        waypoint_transform = None
        if waypoint_location is not None:
...
def initialize_runtime(
    ...
    **_,
    ...
    if hazard_name:
        ...
        if (
            world is not None
            and global_planner is not None
