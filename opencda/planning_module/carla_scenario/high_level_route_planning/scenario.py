...
from utility import behavior_lane_id, waypoint_position, world_point
...
def _find_world_object_by_name(world, carla, object_name: str):
    ...
    environment_match = _find_environment_object_by_name(world, carla, object_name_lower)
...
def _resolve_exact_obstacle_transform(world, carla, marker_name: str):
...
def _resolve_workzone_object(
    *,
    ...
    for candidate_name in _normalized_name_candidates(candidate_names):
        world_object = _find_environment_object_by_name(world, carla, candidate_name)
...
def _nearest_driving_waypoint_info(
    *,
    ...
    if object_location is None or global_planner is None:
        return None
    waypoint = global_planner.get_waypoint(world_point(object_location))
    waypoint_location = waypoint_position(waypoint)
...
def _normalized_route_points_xy(route_points: Sequence[Sequence[object]] | None) -> List[List[float]]:
    return [
...
def _route_progress_m(route_points: Sequence[Sequence[object]] | None, point_xy: Sequence[object] | None) -> float | None:
    if not isinstance(point_xy, Sequence) or len(point_xy) < 2:
        return None
    normalized_route_points = _normalized_route_points_xy(route_points)
    ...
    for segment_start_xy, segment_end_xy in zip(
        ...
        segment_length_sq_m = dx_m * dx_m + dy_m * dy_m
        if float(segment_length_sq_m) <= 1.0e-9:
            projection_t = 0.0
            closest_x_m = float(ax_m)
            closest_y_m = float(ay_m)
            segment_length_m = 0.0
        else:
            projection_t = (
                (float(px_m) - float(ax_m)) * float(dx_m)
                + (float(py_m) - float(ay_m)) * float(dy_m)
            ) / float(segment_length_sq_m)
            projection_t = max(0.0, min(1.0, float(projection_t)))
            closest_x_m = float(ax_m) + float(projection_t) * float(dx_m)
            closest_y_m = float(ay_m) + float(projection_t) * float(dy_m)
            segment_length_m = float(math.sqrt(segment_length_sq_m))
        distance_m = float(math.hypot(float(px_m) - float(closest_x_m), float(py_m) - float(closest_y_m)))
        progress_m = float(cumulative_m) + float(projection_t) * float(segment_length_m)
        if float(distance_m) < float(best_distance_m):
            best_distance_m = float(distance_m)
            best_progress_m = float(progress_m)
        cumulative_m += float(segment_length_m)
    return float(best_progress_m)
...
def _workzone_longitudinal_distance_m(
    *,
    route_points: Sequence[Sequence[object]] | None,
    ego_transform: Any,
    workzone_position_xy: Sequence[object] | None,
) -> float | None:
    ego_position_xy = _transform_xy(ego_transform)
    if ego_position_xy is None:
        return None
    ego_progress_m = _route_progress_m(route_points, ego_position_xy)
    workzone_progress_m = _route_progress_m(route_points, workzone_position_xy)
    if ego_progress_m is None or workzone_progress_m is None:
        return None
    return float(workzone_progress_m) - float(ego_progress_m)
...
def spawn_obstacles(
    ...
    **_,
    ...
    for marker_name in marker_names:
        marker_transform = _resolve_exact_obstacle_transform(world, carla, marker_name)
...
def initialize_runtime(
    ...
    **_,
    ...
    del wall_time_s
    cooperative_message_trigger_longitudinal_distance_m = max(
        0.0,
        float(runtime_cfg.get("cooperative_message_trigger_longitudinal_distance_m", 15.0))
    )
    return {
        ...
        "last_workzone_longitudinal_distance_m": None,
...
def maybe_replan_global_route(
    ...
    **kwargs,
    ...
    longitudinal_trigger_distance_m = max(
        0.0,
        float(next_runtime_state.get("cooperative_message_trigger_longitudinal_distance_m", 15.0)),
    )
    longitudinal_distance_m = _workzone_longitudinal_distance_m(
        route_points=kwargs.get("active_global_route_points", None),
        ego_transform=kwargs.get("ego_transform", None),
        workzone_position_xy=workzone_position_xy,
    )
    next_runtime_state["last_workzone_longitudinal_distance_m"] = (
        None if longitudinal_distance_m is None else float(longitudinal_distance_m)
    )
    if longitudinal_distance_m is None:
        return None, None, next_runtime_state
    if float(longitudinal_distance_m) < 0.0 or float(longitudinal_distance_m) > float(longitudinal_trigger_distance_m):
        return None, None, next_runtime_state
