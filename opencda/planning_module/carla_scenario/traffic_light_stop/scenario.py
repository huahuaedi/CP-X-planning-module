    ...
    **_,
    ...
    for marker_name in marker_names:
        ...
        if global_planner is not None:
            try:
                spawn_waypoint = global_planner.get_waypoint(world_point(marker_transform.location))
            except Exception:
                spawn_waypoint = None
            waypoint_location = waypoint_position(spawn_waypoint)
            if waypoint_location is not None:
                rotation_transform = carla.Transform(
                    ...
                    carla.Rotation(yaw=math.degrees(float(spawn_waypoint.heading or 0.0))),
                )
