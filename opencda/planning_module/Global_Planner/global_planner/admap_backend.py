...
def ensure_runtime_ready(ad_map_install_root: Optional[Union[str, Path]] = None):
...
def load_open_drive_map(xodr_path: Union[str, Path], overlap_margin: float = 0.05) -> List[int]:
...
def load_adm_map(adm_config_path: Union[str, Path]) -> bool:
...
def save_adm_map(adm_path: Union[str, Path]) -> bool:
...
def get_lane_heading(lane_id: int, parametric_offset: float) -> Optional[float]:
...
def get_lane_width_m(lane_id: int, parametric_offset: float) -> Optional[float]:
...
def get_same_direction_adjacent_lane_id(lane_id: int, side) -> Optional[int]:
...
def get_opendrive_lane_info(ad_lane_id: int) -> Dict[str, Optional[int]]:
