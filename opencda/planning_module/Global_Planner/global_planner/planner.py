...
def _serialized(method):
    """Run one AD-map operation while holding the shared planner lock."""

    @wraps(method)
    def locked_call(self, *args, **kwargs):
        with self._lock:
...
class GlobalPlanner:
    ...
    def __init__(
        ...
        ad_map_install_root: Optional[Union[str, Path]] = None,
    ) -> None:
        ...
        self._cache_paths: Optional[Dict[str, Path]] = None
    ...
    @_serialized
    def get_waypoint(self, position, search_radius_m: Optional[float] = None) -> Optional[Waypoint]:
    ...
    def trace_route(
        ...
        lane_change_penalty_m: Optional[float] = None,
        ...
        if bool(activate_route):
            self.active_route = route
        return route

    @property
    def blocked_lane_ids(self):
        """Return a copy so callers cannot change the list by mistake."""

        with self._lock:
            return tuple(int(lane_id) for lane_id in self.blocked_lanes)

    def add_blocked_lane(self, lane_id: int) -> bool:
        """Add one AD-map lane to the blocked list.

        Returns True when the lane was added and False when it was already
        blocked. Route searches automatically skip every lane in this list.
        """

        normalized_lane_id = int(lane_id)
        with self._lock:
            if normalized_lane_id in self.blocked_lanes:
                return False
            if normalized_lane_id not in self._lane_cache:
                raise KeyError("Unknown AD lane id: {}".format(normalized_lane_id))
            self.blocked_lanes.append(normalized_lane_id)
        return True

    def clear_blocked_lanes(self) -> None:
        """Clear blocked lanes when a new scenario starts."""

        with self._lock:
            self.blocked_lanes = []

    @_serialized
    def get_lane_context(self, position, search_radius_m: Optional[float] = None) -> dict:
        """Return the road and neighboring-lane data near one world point."""

        waypoint = self.get_waypoint(position, search_radius_m=search_radius_m)
        if waypoint is None:
            return {
                "found": False,
                "ad_lane_id": 0,
        ...
        # Walk to the right edge, then list lanes from right to left. This is
        # the same ordering expected by the behavior planner.
        rightmost = waypoint
        ...
        lane_waypoints = [rightmost]
        while True:
            candidate = lane_waypoints[-1].left()
            if candidate is None or int(candidate.ad_lane_id) in {
                int(item.ad_lane_id) for item in lane_waypoints
            }:
                break
            lane_waypoints.append(candidate)

        behavior_lane_by_ad_lane = {
            int(item.ad_lane_id): int(index + 1)
            for index, item in enumerate(lane_waypoints)
        }
        behavior_lane_by_opendrive_lane = {
            int(item.lane_id or 0): int(index + 1)
            for index, item in enumerate(lane_waypoints)
        }
        behavior_lane_id = int(
            behavior_lane_by_ad_lane.get(int(waypoint.ad_lane_id), 0)
        )
        return {
            "found": True,
            ...
            "lane_id": behavior_lane_id,
            "lane_ids": [int(index + 1) for index in range(len(lane_waypoints))],
            "ad_lane_ids": [int(item.ad_lane_id) for item in lane_waypoints],
            "behavior_lane_by_ad_lane": behavior_lane_by_ad_lane,
            "behavior_lane_by_opendrive_lane": behavior_lane_by_opendrive_lane,
    ...
    @_serialized
    def find_opt_lane(self, position, search_radius_m: Optional[float] = None) -> Optional[int]:
    ...
    @_serialized
    def _get_adjacent_waypoint(self, waypoint: Waypoint, side: str) -> Optional[Waypoint]:
    ...
    def _make_waypoint_from_lane_offset(
        ...
        enu_position: Optional[Tuple[float, float, float]] = None,
    ) -> Waypoint:
        ...
        enu_heading = backend.get_lane_heading(lane_id, parametric_offset)
        # Positions exposed by Waypoint use the world frame, whose y-axis is
        # reflected from ENU. Reflect the heading as well so it describes the
        # direction of travel in the same frame as Waypoint.position.
        world_heading = (
            None
            if enu_heading is None
            else (float(-enu_heading) + math.pi) % (2.0 * math.pi) - math.pi
        )
        return Waypoint(
