...
from utility.custom_planner import behavior_lane_id, waypoint_position, world_point
...
def find_relevant_signal_context(
    *,
    ...
    for candidate_actor in candidate_actors:
        ...
        if stop_target_xy is not None:
            if actor_location is not None:
                ...
                if global_planner is not None:
                    try:
                        actor_waypoint = global_planner.get_waypoint(world_point(actor_location))
                    except Exception:
                        actor_waypoint = None
                    if actor_waypoint is not None:
                        actor_road_id = int(getattr(actor_waypoint, "road_id", 0) or 0)
                        actor_section_id = int(getattr(actor_waypoint, "section_id", 0) or 0)
                        actor_lane_id = int(behavior_lane_id(actor_waypoint))
                        road_match = stop_target_road_id is not None and actor_road_id == stop_target_road_id
                        section_match = stop_target_section_id is not None and actor_section_id == stop_target_section_id
                        lane_match = stop_target_lane_id is not None and actor_lane_id == stop_target_lane_id
                        if road_match and section_match and lane_match:
                            lane_match_rank = 0
                        elif road_match and lane_match:
                            lane_match_rank = 1
                        elif road_match and section_match:
                            lane_match_rank = 2
                        elif road_match:
                            lane_match_rank = 3
                        if lane_match_rank < 9:
                            signal_source = "custom_Global_Planner_lane_match"
        elif signal_distance_m is not None:
            match_distance_m = float(signal_distance_m)

        if candidate_actor is ego_associated_actor and stop_target_xy is None:
        ...
        if signal_source in {"actor_position_match", "custom_Global_Planner_lane_match"}:
        ...
        candidate_context = {
            ...
            "signal_match_rank": lane_match_rank,
        ...
        source_priority = {
            "custom_Global_Planner_lane_match": 0,
            "actor_position_match": 1,
            "ego_vehicle_association": 2,
        }
        current_priority = (
...
def find_stop_target_from_ego(
    *,
    ...
    ego_point = world_point(ego_position)
    if ego_point is None:
        _clear_stop_target_query_state(query_key)
        return None
    ego_x_m = float(ego_point["x"])
    ego_y_m = float(ego_point["y"])
    ego_z_m = float(ego_point["z"])
    ...
    for idx, route_point in enumerate(route_points):
        ...
        route_waypoint = global_planner.get_waypoint(
            {"x": float(route_point[0]), "y": float(route_point[1]), "z": ego_z_m}
        )
        ...
        if bool(getattr(route_waypoint, "is_intersection", False)):
            ...
            if stop_distance_m <= 1.0e-3:
                return None
            stop_position = waypoint_position(previous_waypoint)
            if stop_position is None:
                return None
            return {
                "x_m": float(stop_position["x"]),
                "y_m": float(stop_position["y"]),
                "heading_rad": float(previous_waypoint.heading or 0.0),
                "lane_id": int(behavior_lane_id(previous_waypoint)),
