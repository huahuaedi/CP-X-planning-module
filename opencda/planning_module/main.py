...
def launch_carla_server(carla_cfg: Mapping[str, Any]) -> subprocess.Popen[bytes]:
    ...
    requested_map_name = str(carla_cfg.get("map", ""))
    map_name = _build_opt_map_fallback(requested_map_name, carla_root=carla_root) or requested_map_name
...
def _candidate_map_leaf(map_name: str) -> str:
    """Return the leaf map token without any `.umap` suffix.

    input: `map_name` (`str`)
    output: normalized map leaf (`str`)
    """

    leaf = str(map_name or "").strip().rstrip("/").split("/")[-1]
    if leaf.lower().endswith(".umap"):
        return leaf[:-5]
    return leaf


def _opendrive_path_for_map(carla_root: str, map_name: str) -> str:
    """Build the expected OpenDRIVE companion file path for one map name.

    input: `carla_root` (`str`), `map_name` (`str`)
    output: candidate `.xodr` path (`str`)
    """
    ...
    return os.path.join(
        ...
        "Maps",
        "OpenDrive",
        f"{_candidate_map_leaf(map_name)}.xodr",
    )


def _build_opt_map_fallback(map_name: str, carla_root: str | None = None) -> str | None:
    """Return the non-`_Opt` map variant when it has the available OpenDRIVE file.

    input: `map_name` (`str`), `carla_root` (`str | None`)
    output: fallback map candidate or no result (`str | None`)
    """

    normalized = str(map_name or "").strip()
    leaf = normalized.rstrip("/").split("/")[-1]
    leaf_without_suffix = _candidate_map_leaf(leaf)
    if not leaf_without_suffix.endswith("_Opt"):
        return None

    if carla_root is not None:
        if os.path.isfile(_opendrive_path_for_map(carla_root, normalized)):
            return None
        base_leaf = leaf_without_suffix[:-4]
        if not os.path.isfile(_opendrive_path_for_map(carla_root, base_leaf)):
            return None
    else:
        base_leaf = leaf_without_suffix[:-4]

    replacement_leaf = leaf.replace(leaf_without_suffix, base_leaf, 1)
    return normalized[: len(normalized) - len(leaf)] + replacement_leaf


def _build_map_load_candidates(map_name: str, carla_root: str | None = None) -> list[str]:
    """
    ...
    fallback_candidate = _build_opt_map_fallback(normalized, carla_root=carla_root)
    if fallback_candidate is not None:
        add(fallback_candidate)
    add(normalized)
...
def run_carla_scenario(name: str) -> int:
    ...
    load_candidates = _build_map_load_candidates(
        requested_map,
