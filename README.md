...
The code is ready locally, but GitHub SSH authentication is not configured. Add this machine's SSH public key to GitHub or push through HTTPS using a Personal Access Token.
...
- SUMO scenarios may require extra local SUMO/netconvert setup.
...
- Some scenario YAML files still contain local CARLA path defaults. Prefer setting `CARLA_ROOT` in the shell and update scenario YAML paths if your machine layout differs.

## MDrive Integration Direction

The intended next step is to wrap the MDrive planner as an adapter rather than directly replacing the MPC internals first. A practical integration path is:

1. Keep the current OpenCDA/CARLA scenario runner.
2. Add an adapter around the MDrive planner interface.
3. Feed ego state, obstacle predictions, route context, and map context into the adapter.
4. Convert the MDrive planner output into the same trajectory/control format consumed by the current runner.
5. Compare MDrive against the current rule-based behavior planner plus MPC using the metric outputs.
